...
from aban.exceptions import Unauthorized, BadRequest
...
class Register(CreateAPIView):
    ...
    def perform_create(self, serializer: UserSerializer):
        if User.objects.filter(mobile = serializer.validated_data.get('mobile')):
            raise BadRequest(detail='user with this mobile number already exists', code=status.HTTP_400_BAD_REQUEST)
...
class Login(GenericAPIView):
    ...
    def post(self, request):
        ...
        raise Unauthorized(detail='Invalid credential', code=status.HTTP_401_UNAUTHORIZED)